<script lang="ts">
	...
	onMount(async () => {
		...
		unpublishedBuffers = JSON.parse(localStorage.getItem('unpublishedBuffers')) || [];
	});
	...
	async function handleSave() {
		...
		unpublishedBuffers = [...unpublishedBuffers, buffer];
...
<div class="relative">
	...
	{#if state == 'saved' && unpublishedBuffers.length > 0 && wallet}
		<div
			class="inline-block"
			transition:fly={{ delay: 0, duration: 300, x: -100, y: 0, opacity: 0.5, easing: quintOut }}
		>
			<Publish {state} bytes={unpublishedBuffers} on:published={clearBuffer} {wallet} />
		</div>
	{/if}
</div>
<slot {handleChange} />
